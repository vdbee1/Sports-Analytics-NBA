...
\begin{aligned}
 ...
 0.59180*X3PA+3.34951*ORB-0.27441*DRB-0.44821*TRB-0.63374*AST-0.07938*ORB:DRB+ϵ\\
\end{aligned}
$$
...
$$
\begin{aligned}
 \hat{Offensive Rating} &=119.438663+2.073357*PACE+1.374430*FG-2.233373*FGA+0.610792*X3PA+1.395365*ORB-0.606648*DRB-4.691143*AST-0.042626*Pace:FG+0.027061*FG:FGA+0.104209*FG:ASTϵ\\
\end{aligned}
$$
...
## First Order Model
We build our first order model including the predictiors that passed the Colinearity Test
```{r}
win_reduced_model = lm(W~SRS+Rel.Pace+ORtg+Rel.ORtg+DRtg+Rel.DRtg+No_of_Awards, data = win_data)
summary(win_reduced_model)
```
## Award Interaction Model
Finding Interaction Terms to Better our Model
```{r}
win_interaction_full_model = lm(W~(SRS+Rel.Pace+ORtg+Rel.ORtg+DRtg+Rel.DRtg+No_of_Awards)^2, data = win_data)
...
```{r}
win_interaction_reduced_model = lm(W~SRS+Rel.Pace+ORtg+Rel.ORtg+DRtg+Rel.DRtg+No_of_Awards+(SRS*Rel.Pace)+(ORtg*Rel.ORtg)+(ORtg*DRtg)+(ORtg*Rel.DRtg)+(Rel.ORtg*DRtg)+(Rel.ORtg*Rel.DRtg)+(DRtg*Rel.DRtg), data = win_data)
summary(win_interaction_reduced_model)
```

## Award Reduced Interaction Model Result:
We can see that we have improved our accuracy by nearly 4% from our previous first order model with Adj R accuracy bumping from  73% to 77% which is significant and we will keep our interaction terms for our final model. Then we will do our prediction in the next step.
...
+ 4.376027 DRtg 
-10.382026 Rel.DRtg 
+1.737722 No_of_Awards      
+ 0.033244 SRS:Rel.Pace\\
-0.009474  ORtg:Rel.ORtg  
-0.056551 ORtg:DRtg
+ 0.162310ORtg:Rel.DRtg\\
+0.031470Rel.ORtg:DRtg   
-0.147104 Rel.ORtg:Rel.DRtg  \\
 -0.071354DRtg:Rel.DRtg      
...
```{r}
overalldata = data.frame(SRS=5.52,Rel.Pace=0.2,ORtg=112.5,Rel.ORtg=0.5,DRtg=106.9,Rel.DRtg=-5.1,No_of_Awards=0)
predict(win_interaction_reduced_model,overalldata,interval="predict")
```
## Award Prediction Result
The actual games won by Golden State Warriors for Season 2021-2022 is 55 Wins and our model predicted 45 Wins. This tell us that our Model Predicted the Actual Wins with a 19% Margin of Error. In future we can train this model with more data to give us better predictions
...
```
## Interpretation
As we can see that our full model our Adj R^2 value was 0.66. And after doing the interaction model, our final model is giving an output of Adj R^2 of 0.71. Which is better that before because our accuracy of predicting the win shares went up by 4%
...
\hat{WinShares}&=
\\0.146916X3PA-0.665229X3P -0.906726FG-0.140737FGA+0.256993ORB\\
+0.127581DRB+0.153601STL+0.171354AST+0.199997 BLK-0.261396TOV
+0.082807PF+0.690488PTS-0.210380FT -0.238074FTA\\
-0.278108  (X3PA*DRB)
+0.173305(X3PA*STL) 
+0.049326 (X3PA*FGA)  \\
-0.191485(X3PA*FG) 
...
```{r}
library(mctest)
injstats_fullmodel<-lm(Salary~X3P+X2P+WS+times_injured+FGA+FG+DRB+STL+AST+BLK+PTS+FTA, data=injurstat) # Full Model with all variables
...
## Full Model
```{r}
injstats_fullmodel<-lm(Salary~X3P+times_injured+FGA+FG+X2P+AST+FTA+DRB+PTS, data=injurstat) # Full Model with all variables
summary(injstats_fullmodel)
```

## First order (significant values in full model)
```{r}
first_order_injstat <- lm(Salary~times_injured+DRB+AST, data=injurstat)
summary(first_order_injstat)
```

## STEP WISE METHOD:
```{r}
injstatstepmod=ols_step_forward_p(injstats_fullmodel, prem = 0.05, details=FALSE)
