...
for season in years:
    ...
    pergame_stats = [[td.getText() for td in pergame[i].findAll('td')] for i in range(0,len(pergame))]
    pergame_header = [[th.getText() for th in pergame[i].findAll('th')] for i in range(0,len(pergame))]

    total_stats = total_stats[1:]
    pergame_stats = pergame_stats[1:]
    pergame_stat = pd.DataFrame(pergame_stats,columns = ['Team', 'G', 'MP', 'FG', 'FGA', 'FG%', '3P', '3PA', '3P%', '2P', '2PA', '2P%', 'FT', 'FTA', 'FT%', 'ORB', 'DRB', 'TRB', 'AST', 'STL', 'BLK', 'TOV', 'PF', 'PTS'])
